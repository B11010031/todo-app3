  ...
  const Nav=()=>(
    <div style={s.nav}>
      {(['today','all','lists','cal'] as Tab[]).map((t,i)=>(
        <button key={t} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',paddingTop:8,paddingBottom:6,gap:2,border:'none',background:'none',cursor:'pointer',color:tab===t?P:'#B0B8CC',fontSize:9,fontFamily:'inherit',fontWeight:500}} onClick={()=>setTab(t)}>
          <Ico n={['sun','list','grid','calendar'][i]} size={22} color={tab===t?P:'#B0B8CC'}/>
          {['今日','所有','清單','行事曆'][i]}
        </button>
      ))}
    </div>
          ...
          {tab==='today'&&(
            <div style={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
              ...
              <div style={s.scroll}>
                ...
                <div style={s.pinDiv}><div style={{flex:1,height:.5,background:'#D4D6E4'}}/><span style={{fontSize:9.5,color:'#B0B8CC',whiteSpace:'nowrap',display:'flex',alignItems:'center',gap:3}}><Ico n="list" size={9} color="#B0B8CC"/>今日任務</span><div style={{flex:1,height:.5,background:'#D4D6E4'}}/></div>
        ...
        <div style={s.overlay} onClick={e=>{if(e.target===e.currentTarget)closePicker();}}>
          <div style={s.pkSheet} onClick={e=>e.stopPropagation()}>
            ...
            {pickerField==='date'&&(()=>{
              ...
              const now=new Date();
              return(<>
                <div style={{fontSize:13,fontWeight:700,color:'#1A1D2E',padding:'0 14px 9px',borderBottom:'.5px solid #ECEEF5'}}>選取時間</div>
                ...
                <div style={{padding:'12px 14px',display:'flex',gap:10}}>
                  {[{id:'mo',label:'月',opts:Array.from({length:12},(_,i)=>({v:i+1,l:`${i+1}月`})),def:now.getMonth()+1},{id:'day',label:'日',opts:Array.from({length:31},(_,i)=>({v:i+1,l:`${i+1}`})),def:now.getDate()}].map(col=>(
            ...
            {pickerField==='pri'&&(<>
              <div style={{fontSize:13,fontWeight:700,color:'#1A1D2E',padding:'0 14px 9px',borderBottom:'.5px solid #ECEEF5'}}>選取優先級</div>
